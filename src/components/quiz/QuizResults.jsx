...
import { getQuestionText } from './quizUtils';
import { Button } from '@/components/ui/button';
import { 
  Trophy, RotateCcw, ChevronDown, ChevronUp, 
  CheckCircle, XCircle, Brain, Target 
} from 'lucide-react';
...
function QuestionReview({ item, index, mode }) {
  const [expanded, setExpanded] = useState(false);
  const questionText = getQuestionText(item.question);
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-100 pt-4 space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-700 bg-white rounded-lg p-2 border border-emerald-100">
          ...
          {mode === 'ai' && item.aiScores && (
            <div className="bg-white rounded-lg p-3 border border-slate-100">
              ...
              <ScoreBar label="Relevance to Question" value={item.aiScores.relevance} color="bg-blue-400" />
              <ScoreBar label="Similarity to Model Answer" value={item.aiScores.similarity} color="bg-violet-400" />
              <ScoreBar label="Technical Accuracy" value={item.aiScores.technical_accuracy} color="bg-emerald-400" />
              <ScoreBar label="Syntax & Terminology" value={item.aiScores.syntax_terminology} color="bg-amber-400" />
              <ScoreBar label="Overall Score" value={item.aiScores.overall_score} color="bg-indigo-500" />
...
export default function QuizResults({ results, config, onRestart }) {
      ...
      <div className={cn("rounded-2xl p-6 mb-6 border", gradeInfo.bg, "border-slate-200")}>
        ...
        {mode === 'ai' && (
          <div className="mt-4 pt-4 border-t border-slate-200">
            <p className="text-xs font-semibold text-violet-600 mb-3 flex items-center gap-1">
              <Brain className="w-3.5 h-3.5" /> Overall AI Score Breakdown
            </p>
            {[
              { label: 'Avg Relevance', key: 'relevance', color: 'bg-blue-400' },
              { label: 'Avg Similarity', key: 'similarity', color: 'bg-violet-400' },
              { label: 'Avg Technical Accuracy', key: 'technical_accuracy', color: 'bg-emerald-400' },
              { label: 'Avg Syntax & Terminology', key: 'syntax_terminology', color: 'bg-amber-400' },
            ].map(({ label, key, color }) => {
              const avg = Math.round(
                evaluated.filter(e => e.aiScores).reduce((s, e) => s + (e.aiScores[key] || 0), 0) /
                (evaluated.filter(e => e.aiScores).length || 1)
              );
              return <ScoreBar key={key} label={label} value={avg} color={color} />;
            })}
          </div>
        )}
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-2">
          {evaluated.map((item, i) => (
            <QuestionReview key={i} item={item} index={i} mode={mode} />
          ))}
        </div>
      </div>
